...
function makeSix() {
	...
	.then((res) => res.json())
	.then((data) => {
		console.log('Свойства объекта: ' +'activity - ' + data.activity + ', ' + 'type - ' + data.type + ', ' + 'price - ' + data.price + ', ' + 'accessibility - ' + data.accessibility);
	})
	.catch((err) => {
		console.log('Ошибка. Запрос не выполнен: ', err);
	});
...
function makeFourteen() {
	...
	fetch('https://official-joke-api.appspot.com/random_joke')
		.then((response) => response.json())
		.then((data) => {
			resultElement.innerHTML = `Setup: ${data.setup}, Punchline: ${data.punchline}.`
		})
